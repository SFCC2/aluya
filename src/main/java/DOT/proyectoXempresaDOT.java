 ...
 * @author Sulay2016
 */
public class proyectoXempresaDOT implements DAO {

    private ArrayList<VOproyectoXempresa> bdVOproyectoXarchivos;

    public proyectoXempresaDOT() {
        this.bdVOproyectoXarchivos = new ArrayList<>();
    }

    @Override
    public boolean insert(Object t) {
        VOproyectoXempresa proyecto = null;
        proyecto.cast(t);

        if (this.search(proyecto.getIdProem()) == null) {
    ...
    public boolean update(int id, Object t) {
        ...
        for (int i = 0; i < this.bdVOproyectoXarchivos.size(); i++) {
            VOproyectoXempresa aux = this.bdVOproyectoXarchivos.get(i);
            if (aux.getIdProyecto() == id) {
                this.bdVOproyectoXarchivos.get(i).setIdEmpresa(proyecto.getIdEmpresa());;
    ...
    public Object search(int id) {
        for (VOproyectoXempresa inventario : bdVOproyectoXarchivos) {
